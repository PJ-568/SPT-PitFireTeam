...
- Out of combat, patrol healing looks for a nearby cover point before starting `HealAction`, then uses the same sprinting `runToHeal` cover action as combat healing. If no valid cover is found within 60m, healing can fall back to the current position. Followers say `StartHeal` once per patrol heal sequence: when they start moving to found cover, or when they start healing if no cover movement is available.
- Post-combat full recovery no longer relies on repeated manual first-aid top-off. Combat handoff arms recovery state, the first patrol `HealAction` starts a 12 second recovery window, vanilla first aid/surgery can run during that window, then patrol waits for the main weapon to be selected and ready, force-restores health through the same path as the explicit force-heal hotkey, clears the post-combat flag, and resumes movement.
